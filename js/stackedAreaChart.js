...
StackedAreaChart.prototype.initVis = function(){
    ...
    vis.area = d3.area()
        .x(function(d) { return vis.x(parseTime(d.data.Wave)); })
...
StackedAreaChart.prototype.wrangleData = function(){
    ...
    vis.dayNest = d3.nest()
        .key(function(d){ return formatTime(d.Wave); })
        .entries(calls)
    ...
    vis.dataFiltered = vis.dayNest
        .map(function(day){
            return day.values.reduce(function(accumulator, current){
                ...
                accumulator[current.Country] = accumulator[current.Country] + current[vis.variable]
                return accumulator;
            }, {
                "Americas": 0,
                "Europe": 0,
...
StackedAreaChart.prototype.updateVis = function(){
    ...
    vis.maxDateVal = d3.max(vis.dataFiltered, function(d){
        var vals = d3.keys(d).map(function(key){ return key !== 'Wave' ? d[key] : 0 });
        return d3.sum(vals);
    });

    // Update scales
    vis.x.domain(d3.extent(vis.dataFiltered, (d) => {  return parseTime(d.Wave); }));
    ...
    vis.teams = vis.g.selectAll(".Country")
...
StackedAreaChart.prototype.addLegend = function(){
    ...
    var legendArray = [
        {label: "Europe", color: vis.color("Europe")},
        {label: "Asia", color: vis.color("Asia")},
        {label: "Americas", color: vis.color("Americas")},
        {label: "Africa", color: vis.color("Africa")}
    ]
